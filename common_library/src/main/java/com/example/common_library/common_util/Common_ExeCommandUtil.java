...
/**
 使用方式为2种。
 1.首先是阻塞方式，这种调用方式会一直阻塞至命令执行完成，返回命令行的输出结果
 String str = new ExeCommand().run(cmd, 10000).getResult();


 2.异步方式，这种调用方式会直接返回，之后可以使用 getResult() 获取结果，使用 isRunning() 来判断是否完成，比如
 ...
 * */
public class Common_ExeCommandUtil {
    //shell进程
     ...
     */
    public Common_ExeCommandUtil(boolean synchronous) {
     ...
     * @return this
     */
    public Common_ExeCommandUtil run(String command, final int maxTime) {
